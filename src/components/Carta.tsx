...
export default function Carta({
  ...
  slug: string;
  // null = pedido de mostrador: sin mesa, se pide y se recoge cuando llaman el número.
  code: string | null;
}) {
  const router = useRouter();
  // Clave para recordar el pedido abierto en ESTE celular. Sin mesa, una por local.
  const codeKey = code ?? "mostrador";
  ...
  useEffect(() => {
    const id = recallOrder(slug, codeKey);
    if (!id) return;
    let alive = true;
    fetch(`/api/orders/${id}`)
  ...
  async function submitOrder() {
    ...
    try {
      const res = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          restaurantSlug: slug,
          // Con mesa va el código; sin mesa, el nombre opcional para llamar al cliente.
          ...(code ? { tableCode: code } : { customerName }),
          paymentMethod,
          notes,
      ...
      rememberOrder(slug, codeKey, d.id);
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-x-5 bottom-4 mx-auto flex max-w-xl items-end gap-3.5">
          ...
          >
            {code ? fmt(t.common.table, { n: code }) : t.menu.counterBadge}
          </span>
        </div>
      </div>
              ...
              {!code && (
                <>
                  ...
                  <input
                    ...
                    placeholder={t.menu.counterNamePh}
                    maxLength={60}
                    style={{
                      width: "100%",
