      ...
      <main className="mx-auto w-full max-w-4xl flex-1 px-6 py-16">
        ...
        {TENANTS.map((r) => (
          ...
          >
            ...
            {/* Las pantallas del local ya no cuelgan de aquí: se entra por la puerta
                del restaurante, que es como se entra de verdad. La tablet de la
                cocina no arranca en un índice de Vectaryx — arranca en su casa. */}
            <div className="p-5">
              <Link
                href={`/${r.slug}`}
                className="flex items-center gap-4 px-5 py-4 transition hover:-translate-y-0.5"
                style={{
                  background: r.brand,
                  borderRadius: 16,
                  boxShadow: "0 16px 32px -26px rgba(33,29,24,.5)",
                }}
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-[15px] font-extrabold" style={{ color: "#fff" }}>
                    {r.name} →
                  </span>
                  <span
                  ...
                  >
                    {SURFACES.map((s) => t.nav[s.key]).join(" · ")} · {t.home.pinHint}
                  </span>
                </span>
              </Link>
            </div>
